...
GlobalSerializerRegistrationList serializers; ///< List of packet serializers (ISerializer)
...
void Buffer::writeNBytes(unsigned int length, const void *_src)    // throw OutOfBuffer
{
...
void Buffer::writeUint16(uint16_t data)    // hton, throw OutOfBuffer
{
    if (pos < bufsize)
...
void Buffer::writeUint32(uint32_t data)    // hton, throw OutOfBuffer
{
    if (pos < bufsize)
