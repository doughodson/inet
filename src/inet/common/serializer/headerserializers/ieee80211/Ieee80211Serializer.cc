...
void Ieee80211Serializer::serialize(const cPacket *pkt, Buffer &b, Context& c)
{
    ...
    else if (dynamic_cast<const Ieee80211DataOrMgmtFrame *>(pkt))
    {
        ...
        if (dynamic_cast<const Ieee80211DataFrameWithSNAP *>(pkt))
        {
            ...
            SerializerBase::serialize(encapPacket, b, c, ETHERTYPE, dataFrame->getEtherType(), 4);   // 4 byte for store crc at end of packet
            packetLength = b.getPos();
        }
...
unsigned int Ieee80211Serializer::parseDataOrMgmtFrame(Buffer &b, Ieee80211DataOrMgmtFrame *Frame, short type)
{
    unsigned int offset = 0;
    Frame->setType(type);
    uint8_t fc0 = b.readByte();   // i_fc[0]
...
{
    unsigned char *buf = b._getBuf();
    unsigned int bufsize = b._getBufSize();
    ...
    switch(type)
    {
        ...
        case 0x8: // ST_DATA
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, dataFrame, ST_DATA);

            // snap_header:
            b.accessNBytes(6);
            dataFrame->setEtherType(b.readUint16());    // ethertype

            cPacket *encapPacket = SerializerBase::parse(b, c, ETHERTYPE, dataFrame->getEtherType(), 4);
            ASSERT(encapPacket);
        ...
        case 0xB0: // ST_AUTHENTICATION
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_AUTHENTICATION);
        ...
        case 0xC0: //ST_ST_DEAUTHENTICATION
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_DEAUTHENTICATION);
        ...
        case 0xA0: // ST_DISASSOCIATION
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_DISASSOCIATION);
        ...
        case 0x40: // ST_PROBEREQUEST
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_PROBEREQUEST);
        ...
        case 0x00: // ST_ASSOCIATIONREQUEST
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_ASSOCIATIONREQUEST);
        ...
        case 0x02: // ST_REASSOCIATIONREQUEST
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_REASSOCIATIONREQUEST);
            Ieee80211ReassociationRequestFrameBody body;
            packetLength += 4;

            uint8_t temp[MAC_ADDRESS_SIZE];
            getNByte(temp, MAC_ADDRESS_SIZE, buf + packetLength);
            MACAddress tempMAC;
            tempMAC.setAddressBytes(temp);
            body.setCurrentAP(tempMAC);
            packetLength += MAC_ADDRESS_SIZE;
        ...
        case 0x01: // ST_ASSOCIATIONRESPONSE
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_ASSOCIATIONRESPONSE);
        ...
        case 0x03: // ST_REASSOCIATIONRESPONSE
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_REASSOCIATIONRESPONSE);
        ...
        case 0x80: // ST_BEACON
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_BEACON);
        ...
        case 0x50: // ST_PROBERESPONSE
        {
            ...
            unsigned int packetLength = parseDataOrMgmtFrame(b, pkt, ST_PROBERESPONSE);
