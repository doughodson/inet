...
const ITransmission *Ieee80211ScalarTransmitter::createTransmission(const IRadio *transmitter, const cPacket *macFrame, simtime_t startTime) const
{
    ...
    Ieee80211Modulation modulationType = Ieee80211Mode::getModulationType(opMode, transmissionBitrate.get());
    const simtime_t duration = SIMTIME_DBL(modulationType.calculateTxDuration(macFrame->getBitLength(), preambleMode));
